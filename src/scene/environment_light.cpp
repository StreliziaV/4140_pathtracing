...
namespace CGL { namespace SceneObjects {
  ...
  void EnvironmentLight::init() {
    ...
    double temp_marg = 0.0;
    for (int j = 0; j < h; ++j) {
      for (int i = 0; i < w; ++i) {
        pdf_envmap[w * j + i] = pdf_envmap[w * j + i] / sum;
        temp_marg += pdf_envmap[w * j + i];
      }
      marginal_y[j] = temp_marg;
    }
    for (int j = 0; j < h; ++j) {
      double temp_pre = 0.0;
      if (j != 0) temp_pre = marginal_y[j - 1];
      double temp_cond = 0.0;
      for (int i = 0; i < w; ++i) {
        temp_cond += pdf_envmap[w * j + i];
        conds_y[w * j + i] = temp_cond / (marginal_y[j] - temp_pre);
      }
    }
  ...
  Vector3D EnvironmentLight::sample_L(const Vector3D p, Vector3D* wi,
    ...
    // *distToLight = INF_D;
    // *pdf = 1.0 / (4.0 * PI);
    // Vector2D my_xy = theta_phi_to_xy(dir_to_theta_phi(*wi));
    // return bilerp(my_xy);

    //importance
    uint32_t w = envMap->w, h = envMap->h;
    Vector2D my_s = sampler_uniform2d.get_sample();
    int my_x = w - 1;
    int my_y = h - 1;
    for (int j = 0; j < h; j++) {
      if (this->marginal_y[j] >= my_s.y) {
        my_y = j;
        break;
      }
    }
    for (int i = 0; i < w; i++) {
      if (this->conds_y[my_y * w + i] >= my_s.x) {
        my_x = i;
        break;
      }
    }
    ...
    *distToLight = INF_D;
    Vector2D my_tp = xy_to_theta_phi(Vector2D(my_x, my_y));
    *wi = theta_phi_to_dir(my_tp);
    *pdf = this->pdf_envmap[my_y * w + my_x] * w * h / ((2 * M_PI * M_PI) * sin(my_tp.x));

    return envMap->data[my_y * w + my_x];
  }
  ...
  Vector3D EnvironmentLight::sample_dir(const Ray& r) const {
    ...
    Vector2D my_tp = dir_to_theta_phi(r.d);
    Vector2D my_xy = theta_phi_to_xy(my_tp);
